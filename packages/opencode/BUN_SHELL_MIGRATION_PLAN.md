...
Replace runtime Bun shell template-tag usage in `packages/openloom/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/openloom/src` is removed except:
