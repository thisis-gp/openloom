...
it.instance("plan agent denies edits except .openloom/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".openloom/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".openloom", "skill", "perm-skill")
...
`,
      ...
      process.env.OPENLOOM_TEST_HOME = test.directory
