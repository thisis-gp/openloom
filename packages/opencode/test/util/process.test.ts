...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.OPENLOOM_TEST ?? "")'), {
      env: {
        OPENLOOM_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "OPENLOOM_TEST_SHELL"], {
      shell: true,
      env: {
        OPENLOOM_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("OPENLOOM_TEST_SHELL=ok")
  })
