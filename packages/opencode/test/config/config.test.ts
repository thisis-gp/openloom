...
const managedConfigDir = process.env.OPENLOOM_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "openloom.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "openloom.json") {
...
test("does not create global config when OPENLOOM_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.OPENLOOM_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.OPENLOOM_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "openloom.jsonc"))).toBe(false)
  } finally {
    ;(Global.Path as { config: string }).config = prevConfig
    if (prevEnv === undefined) delete process.env.OPENLOOM_CONFIG_DIR
    else process.env.OPENLOOM_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "openloom.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "openloom.jsonc")
  ...
  try {
    ...
    await withTestInstance({
      ...
      fn: async (ctx) => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "openloom.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["OPENLOOM_CONSOLE_TOKEN"]
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["OPENLOOM_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["OPENLOOM_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "openloom.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".openloom", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("does not try to install dependencies in read-only OPENLOOM_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.OPENLOOM_CONFIG_DIR
  process.env.OPENLOOM_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OPENLOOM_CONFIG_DIR
    else process.env.OPENLOOM_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OPENLOOM_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".openloom")
...
// Note: preload.ts sets OPENLOOM_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
    ...
    if (urlStr.includes(".well-known/opencode")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/openloom.json",
              headers: {
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.openloom/plugin/demo.ts", "file:///project/.openloom/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.openloom/plugin/demo.ts"])
  })
...
describe("OPENLOOM_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OPENLOOM_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OPENLOOM_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OPENLOOM_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENLOOM_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OPENLOOM_CONFIG_DIR"]
      process.env["OPENLOOM_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OPENLOOM_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OPENLOOM_CONFIG_DIR"]
      } else {
        process.env["OPENLOOM_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OPENLOOM_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OPENLOOM_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENLOOM_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OPENLOOM_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OPENLOOM_CONFIG_DIR"] = configDirTmp.path

      await withTestInstance({
        directory: projectTmp.path,
        fn: async (ctx) => {
          const config = await load(ctx)
          // Should load from OPENLOOM_CONFIG_DIR, not project
...
describe("OPENLOOM_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENLOOM_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENLOOM_CONFIG_CONTENT"]
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OPENLOOM_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OPENLOOM_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OPENLOOM_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENLOOM_CONFIG_CONTENT"]
