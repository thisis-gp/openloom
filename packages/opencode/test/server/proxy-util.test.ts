...
describe("ProxyUtil", () => {
  ...
  describe("headers", () => {
    ...
    test("strips openloom-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          "x-openloom-directory": "/home/user/project",
          "x-openloom-workspace": "ws_123",
          "accept-encoding": "gzip",
          "x-custom": "keep",
        },
      })
      const result = ProxyUtil.headers(req)
      expect(result.get("x-openloom-directory")).toBeNull()
      expect(result.get("x-openloom-workspace")).toBeNull()
