...
function app(serverPath: ServerPath, input?: { password?: string; username?: string }) {
  Flag.OPENLOOM_SERVER_PASSWORD = input?.password
  Flag.OPENLOOM_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return AppFileSystem.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".openloom", "skills", "project-rest-skill", "SKILL.md"),
      `---
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("openloom", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("openloom", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
