...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.OPENLOOM_WEBSEARCH_PROVIDER

    try {
      process.env.OPENLOOM_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.OPENLOOM_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.OPENLOOM_WEBSEARCH_PROVIDER
      else process.env.OPENLOOM_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for openloom or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderID.openloom, { exa: false, parallel: false })).toBe(true)
