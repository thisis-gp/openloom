...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for openloom.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal openloom sessions
...
openloom acp --cwd /path/to/project
```
...
```bash
OPENLOOM_ENABLE_QUESTION_TOOL=1 openloom acp
```
...
- **Terminal Integration**: Full terminal support via openloom's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | openloom acp
```
...
ACP sessions map cleanly to openloom's internal session model:
