...
import * as EffectLogger from "@openloom/core/effect/logger"
...
import { SessionEvent } from "@openloom/core/session-event"
import { ModelV2 } from "@openloom/core/model"
import { ProviderV2 } from "@openloom/core/provider"
import { AgentAttachment, FileAttachment, ReferenceAttachment, Source } from "@openloom/core/session-prompt"
...
export class Service extends Context.Service<Service, Interface>()("@openloom/SessionPrompt") {}
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const [skills, env, instructions, agentSupp, modelMsgs] = yield* Effect.all([
              sys.skills(agent),
              sys.environment(model),
              instruction.system().pipe(Effect.orDie),
              sys.agentSupplement(agent),
              MessageV2.toModelMessagesEffect(msgs, model),
            ])
            const system = [...env, ...instructions, ...(agentSupp ? [agentSupp] : []), ...(skills ? [skills] : [])]
