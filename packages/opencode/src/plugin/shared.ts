...
export const DEPRECATED_PLUGIN_PACKAGES = ["openloom-openai-codex-auth", "openloom-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, openloomVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(openloomVersion) || semver.major(openloomVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.openloom
  if (typeof range !== "string") return
  if (!semver.satisfies(openloomVersion, range)) {
    throw new Error(`Plugin requires openloom ${range} but running ${openloomVersion}`)
  }
}
