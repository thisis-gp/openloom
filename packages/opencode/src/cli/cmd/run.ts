// CLI entry point for `openloom run`.
//
// Handles three modes:
...
//      openloom server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@openloom/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run openloom with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running openloom server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OPENLOOM_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENLOOM_SERVER_USERNAME or 'openloom')",
      })
      .option("dir", {
        type: "string",
