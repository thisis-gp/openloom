...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in openloom.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  if (!global) {
    candidates.push(path.join(baseDir, ".openloom", "openloom.json"), path.join(baseDir, ".openloom", "openloom.jsonc"))
  }
  ...
  // Default to openloom.json if none exist
  return candidates[0]
}
