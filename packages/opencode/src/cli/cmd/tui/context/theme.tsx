...
import openloom from "./theme/openloom.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@openloom/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "openloom")
        draft.active = typeof active === "string" ? active : "openloom"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.openloom, store.mode)
    })
