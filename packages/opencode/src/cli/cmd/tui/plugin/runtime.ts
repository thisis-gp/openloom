...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".openloom"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".openloom", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".openloom", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved; dispose?: () => void }) {
  ...
  try {
    ...
    const records = Flag.OPENLOOM_PURE ? [] : (config.plugin_origins ?? [])
    if (Flag.OPENLOOM_PURE && config.plugin_origins?.length) {
