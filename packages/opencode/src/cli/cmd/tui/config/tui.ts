...
import type { TuiAttentionSoundName } from "@openloom/plugin/tui"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old openloom.json shape) still get their settings applied.
  ...
  const projectFiles = Flag.OPENLOOM_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit OPENLOOM_TUI_CONFIG override, if set.
  if (Flag.OPENLOOM_TUI_CONFIG) {
    const configFile = Flag.OPENLOOM_TUI_CONFIG
  ...
  for (const dir of dirs) {
    if (!dir.endsWith(".openloom") && dir !== Flag.OPENLOOM_CONFIG_DIR) continue
