...
Delete `packages/openloom/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/openloom/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/openloom/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/openloom`
- TUI plugin loader tests in `packages/openloom` if runtime plugin API wiring changed
