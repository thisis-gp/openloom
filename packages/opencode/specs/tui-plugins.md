...
- Author package entrypoint is `@openloom/plugin/tui`.
...
- Import types from `@openloom/plugin/tui`.
- `@openloom/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@openloom/plugin/tui"
...
That is what makes local config-scoped plugins able to import `@openloom/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@openloom/plugin/tui`.
...
- `--pure` / `OPENLOOM_PURE` skips external TUI plugins only.
