...
Practical reference for a future `packages/server` split after the openloom
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/openloom`.
...
Until enough shared service code lives outside `packages/openloom`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/openloom`

It should not import `packages/openloom` services while `packages/openloom`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on openloom-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/openloom` implementation details.
