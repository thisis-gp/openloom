...
export const layer: Layer.Layer<Service, never, Requirements> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.OPENLOOM_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.OPENLOOM_MODELS_PATH ?? filepath).pipe(
    ...
    if (!Flag.OPENLOOM_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
