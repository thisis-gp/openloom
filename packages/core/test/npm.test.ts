...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@openloom/acme")).toBe("@openloom/acme")
    expect(Npm.sanitize("@openloom/acme@1.0.0")).toBe("@openloom/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/openloom/acme.git"
    const expected = win ? "acme@git+https_//github.com/openloom/acme.git" : spec
