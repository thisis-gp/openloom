...
import { AuthV2 } from "@openloom/core/auth"
import { ModelV2 } from "@openloom/core/model"
import { PluginV2 } from "@openloom/core/plugin"
import { AuthPlugin } from "@openloom/core/plugin/auth"
import { CloudflareWorkersAIPlugin } from "@openloom/core/plugin/provider/cloudflare-workers-ai"
  ...
  it.effect("uses env API key over auth or configured API key and keeps the Cloudflare User-Agent", () =>
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct", CLOUDFLARE_API_KEY: "env-key" }, () =>
      Effect.gen(function* () {
        ...
        expect(headers["user-agent"]).toMatch(/^openloom\/.* cloudflare-workers-ai \(.+\) ai-sdk\/openai-compatible\//)
      }),
    ),
  )
