...
import { type Session } from "@openloom/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("openloom://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("openloom://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("openloom://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("openloom://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("openloom://open-project")).toBeUndefined()
    expect(parseDeepLink("openloom://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "openloom://open-project?directory=/a",
      "openloom://other?directory=/b",
      "openloom://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("openloom://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("openloom://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("openloom://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("openloom://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "openloom://new-session?directory=/a",
      "openloom://open-project?directory=/b",
      "openloom://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENLOOM__: {
        deepLinks: ["openloom://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENLOOM__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["openloom://open-project?directory=/a"])
