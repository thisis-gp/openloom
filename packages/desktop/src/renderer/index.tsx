...
import "./styles.css"
import { useTheme } from "@openloom/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.OPENLOOM_CHANNEL === "prod" &&
...
const deepLinkEvent = "openloom:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__OPENLOOM__ ??= {}
  const pending = window.__OPENLOOM__.deepLinks ?? []
  window.__OPENLOOM__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("openloom.global.dat").getItem("language")
