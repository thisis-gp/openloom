...
const APP_IDS: Record<string, string> = {
  dev: "ai.openloom.desktop.dev",
  beta: "ai.openloom.desktop.beta",
  prod: "ai.openloom.desktop",
}
const TEST_ONBOARDING = process.env.OPENLOOM_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.OPENLOOM_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.openloom.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `openloom-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Openloom Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("openloom://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("openloom")
  registerRendererProtocol()
  setDockIcon()
  setupAutoUpdater()

  const needsMigration = ((): boolean => {
    if (process.env.OPENLOOM_DB === ":memory:") return false

    const xdg = process.env.XDG_DATA_HOME
    const base = xdg && xdg.length > 0 ? xdg : join(homedir(), ".local", "share")
    return !existsSync(join(base, "openloom", "openloom.db"))
  })()
  let overlay: BrowserWindow | null = null

  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.OPENLOOM_PORT
    if (fromEnv) {
