...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.openloom.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `openloom.settings.dat` is special: it maps to the `openloom.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"openloom.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // openloom.settings.dat → the electron settings store ("openloom.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "openloom.global.dat").
  const storeName = filename === "openloom.settings.dat" ? "openloom.settings" : filename
