...
Internal package; depended on as `@openloom/http-recorder` from another
workspace package.

```ts
import { HttpRecorder } from "@openloom/http-recorder"
```
...
```ts
import { HttpRecorder, Redactor } from "@openloom/http-recorder"
