...
console.log("🚀 Starting openloom server...")
const openloom = await createOpencode({
  port: 0,
})
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new openloom session...")
    const { client, server } = openloom
  ...
  console.log("📝 Sending to openloom:", message.text)
