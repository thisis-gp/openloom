...
const model = "openloom/gpt-5.3-codex"
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with openloom...`)
  ...
  try {
    await $`openloom run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  openloom failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with openloom")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with openloom...")
