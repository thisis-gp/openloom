...
brew install anomalyco/tap/openloom # macOS と Linux（推奨。常に最新）
brew install openloom              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S openloom            # Arch Linux (Stable)
paru -S openloom-bin               # Arch Linux (Latest from AUR)
mise use -g openloom               # どのOSでも
nix run nixpkgs#openloom           # または github:anomalyco/openloom で最新 dev ブランチ
```
...
Openloom はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/openloom/releases) から直接ダウンロードするか、[openloom.ai/download](https://openloom.ai/download) を利用してください。
...
1. `$OPENLOOM_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.openloom/bin` - デフォルトのフォールバック
...
Openloom には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://openloom.ai/docs/agents) の詳細はこちら。

### ドキュメント

Openloom の設定については [**ドキュメント**](https://openloom.ai/docs) を参照してください。

### コントリビュート

Openloom に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### Openloom の上に構築する

Openloom に関連するプロジェクトで、名前に "openloom"（例: "openloom-dashboard" や "openloom-mobile"）を含める場合は、そのプロジェクトが Openloom チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。
...
- 100% オープンソース
- 特定のプロバイダーに依存しません。[Openloom Zen](https://openloom.ai/zen) で提供しているモデルを推奨しますが、Openloom は Claude、OpenAI、Google、またはローカルモデルでも利用できます。モデルが進化すると差は縮まり価格も下がるため、provider-agnostic であることが重要です。
- そのまま使える LSP サポート
- TUI にフォーカス。Openloom は neovim ユーザーと [terminal.shop](https://terminal.shop) の制作者によって作られており、ターミナルで可能なことの限界を押し広げます。
- クライアント/サーバー構成。例えば Openloom をあなたのPCで動かし、モバイルアプリからリモート操作できます。TUI フロントエンドは複数あるクライアントの1つにすぎません。

---

**コミュニティに参加** [Discord](https://discord.gg/openloom) | [X.com](https://x.com/openloom)
